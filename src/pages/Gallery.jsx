...
export default function Gallery() {
    ...
    const { allSeason, gallery, loading } = useSelector((state) => state.season);
    ...
    useEffect(() => {
        if (allSeason.length > 0 && activeTab === "") {
            const firstId = allSeason[0].id;
            setActiveTab(firstId);
            dispatch(fetchSeasonGallery(firstId));
        }
    }, [allSeason]);
    ...
    const handleTabChange = (id) => {
        setActiveTab(id);
        dispatch(fetchSeasonGallery(id));
    };
